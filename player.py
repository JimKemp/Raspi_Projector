...
class Player:
    ...
    def play(self, filepath):
        ...
        # On a console-only Pi (no X11), VLC needs an explicit video output
        # that talks to KMS/DRM. config.VLC_VOUT selects it; leave it None when
        # running under a desktop so VLC auto-detects X11.
        if config.VLC_VOUT:
            cmd += ["--vout", config.VLC_VOUT]
        cmd += [
            "--intf", "rc",
